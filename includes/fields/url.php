...
if (!class_exists('\SCF\Fields\SCFUrl')) {
	...
	class SCFUrl {
		...
		function __construct() {
			...
			add_action( 'scf_field_url', array($this, 'displayHtml') );
			add_action( 'scf_field_url', array($this, 'displayValidation'), 20 );
		}

		/**
		 * Initialise le type Url
		...
		/**
		 * Marque le type Url comme de type Label, afin de l'entourer d'un label
		...
		public function displayHtml($args) {
			...
			if (!isset($input_args['inputmode'])) $input_args['inputmode'] = 'url';
