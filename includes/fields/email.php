...
if (!class_exists('\SCF\Fields\SCFEmail')) {

	/**
	 * Classe du champ Email
	 */
	class SCFEmail {
		...
		function __construct() {
			...
			add_action( 'scf_field_email', array($this, 'displayHtml') );
			add_action( 'scf_field_email', array($this, 'displayValidation'), 20 );
		}

		/**
		 * Initialise le type Email
		...
		/**
		 * Marque le type Email comme de type Label, afin de l'entourer d'un label
		...
		public function displayHtml($args) {
			...
			$input_args['placeholder'] = (isset($args['placeholder'])) ? esc_attr(strval($args['placeholder'])) : '' ;
			$input_args['autocomplete'] = (isset($args['autocomplete'])) ? esc_attr(strval($args['autocomplete'])) : 'on' ;
			...
			if (!isset($input_args['inputmode'])) $input_args['inputmode'] = 'email';
