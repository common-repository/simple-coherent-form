...
if (!class_exists('\SCF\Fields\SCFCheckbox')) {
	...
	class SCFCheckbox {
		...
		function __construct() {
			...
			add_action( 'scf_field_checkbox', array($this, 'displayHtml') );
		}

		/**
		 * Ajoute les arguments par défaut nécessaires au type Checkbox
		...
		/**
		 * Marque le type Checkbox comme de type Block, afin de ne pas l'entourer d'un label
		...
		public function displayHtml($args) {
			...
			/* Récupère la/les valeur(s) de la checkbox */
			$select_values = array('');
			if (isset($args['value'])) {
			...
			if (isset($options) && is_array($options)) {
				...
				if (isset($args['placeholder']) && !empty($args['placeholder'])) {
					$this->displayPlaceholder($args['placeholder'], $inputs_args, $select_values);
				}
				...
				foreach ($options as $option) {
					...
					$input_args = apply_filters('scf_checkbox_input_args', $input_args, $option, $args);

					$item_classes_array = array('scf-checkbox-item');
					if ((!isset($args['placeholder']) || empty($args['placeholder'])) && $i==1) { $item_classes_array[] = 'scf-checkbox-first'; }
					if ($i>=count($options)) $item_classes_array[] = 'scf-checkbox-last';
					...
					$item_classes_array = apply_filters('scf_checkbox_item_classes', $item_classes_array, $option, $args);
					$item_classes = implode(' ', $item_classes_array);

					do_action( 'scf_before_item_checkbox', $option, $i, in_array($option_value, $select_values), $args);

					$this->displayItem(
						$option_label,
						$item_classes,
						$input_args,
						in_array($option_value, $select_values),
						boolval(isset($option['required']) && $option['required'])
					);

					do_action( 'scf_after_item_checkbox', $option, $i, in_array($option_value, $select_values), $args);
				}
			}
		}
		 ...
		 * @return void
		 */
		public function displayPlaceholder($label, $args = array(), $selected = array()) {
			?><label class="scf-checkbox scf-checkbox-placeholder" tabindex="0">
				<input <?php
					foreach ($args as $key => $value) { echo esc_attr($key) . '="' . esc_attr($value) . '" '; }
					checked(in_array('', $selected), true, true);
				?>/>
				<span class="scf-checkbox-label"><?php echo esc_html($label); ?></span>
			</label><?php
		}
		...
		public function displayItem($label, $classes = '', $args = array(), $selected = false, $required = false) {
			?><label class="<?php echo esc_attr($classes); ?>" tabindex="0">
				...
				?>/>
				<span class="scf-checkbox-label"><?php
